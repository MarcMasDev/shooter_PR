    ...
    public class CarAIControl : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            else
            {
                ...
                {
                    fwd = m_Rigidbody.linearVelocity.normalized; //normalize the velocity to not modify the angle
                }
                    ...
                    case BrakeCondition.TargetDirectionDifference:
                        {
                            ...
                            //To make it easier to create the track without rotating the waypoints, try to auto-calculate it
                            Vector3 targetDirection = (m_Target.position - transform.position).normalized;
                            float approachingCornerAngle = Vector3.Angle(targetDirection, fwd);
        ...
        private void OnCollisionStay(Collision col)
        {
            ...
            // anti-stuck logic for ANY collision
            if (m_Rigidbody.linearVelocity.magnitude < 1f)
            {
                m_Rigidbody.AddForce(-transform.forward * 2500f);
            }
        }
